...
function loadDataFilter(idParentCurrent = 'base') {
    let urlAllFilter = BASE_URL + 'Filter/getFilter/' + idParentCurrent
    let tabledataCategories = [];
    ...
    fetchFilterJSON().then(tabledataCategories => {
        tabledataCategories.map((x, index) => {
            // console.log(x)
            x.index = index + 1
        });
        console.log('Debajo está ')
        console.log(tabledataCategories)
        // Table init
        var rowMenu = [{
        ...
        let tableCategories = new Tabulator("#categoriesTable", {
        data: tabledataCategories,
        ...
        rowFormatter: function (row) {
            ...
            let icon = document.createElement('i')
            if (row._row.table.getSelectedRows().length == tabledataCategories.length) {
            ...
            let modal = document.querySelector('.update-categorie-form');
            ...
            updateFilterBtn.onclick = () => {
                ...
                async function fetchFilterParentJSON() {
                    const response = await fetch(BASE_URL + 'Filter/getFilterParent');
                    const categories = await response.json();
                    return categories;
                }
                ...
                fetchFilterParentJSON().then(tree => {
                    ...
                    async function fetchFilterUpdateJSON() {
                        const response = await fetch(BASE_URL + 'Filter/selectOnly/'+row._row.data.id);
                        const categories = await response.json();
                        return categories;
                    }
                            ...
                            req.onreadystatechange = (e) => {
                                if (req.readyState == 4 && req.status == 200) {
                                    ...
                                    if (data.status) {
                                        ...
                                        modal.style.display = 'none';
                                        // loadDataFilter(parent);
                                        loadTreeFilter();
                                        reloadTableFilter(parent);
                                        console.log(categoryParent)
                                        const breadcumbLinks = [...document.querySelectorAll('.breadcumb.breadcumb-categories .links .d-flex.fadeInLeft')] 
                        ...
                        if (item.subcategory.length > 0) {
                            ...
                            <div class="chevrondown-item-gj8">
                                <div class="chevrondown-head-gj8">
                                    ...
                                    <span class="chevrondown-name">${ item.nombre }</span> <span class="category-mark">(Categoría)</span>
                                </div>
                        ...
                        btn.addEventListener('click', (e) => {
                            ...
                            if (arrow != null) {
                                ...
                                if (btn.attributes[1].nodeValue == 'base') {
                                    document.querySelector('.update-categorie-form .input-type').textContent = 'Se guardará como: categoría'
                                } else {
                                    document.querySelector('.update-categorie-form .input-type').textContent = 'Se guardará como: subcategoría'
                                }
                                radioBtns.map(_btn => {
            ...
            if (row._row.data.subcategory.length > 0) {
                ...
                searchSubcategories.onclick = () => {
                    ...
                    addLinkBreadCumb(link, id, subcategory)
                }
            } else {
        ...
        function viewSubCatJson(id) {
            let urlAllFilter = BASE_URL + 'Filter/getFilter/' + id
            async function fetchFilterJSON() {
                const response = await fetch(urlAllFilter);
                const categories = await response.json();
                return categories;
            ...
            fetchFilterJSON().then(subcategories => {
                ...
                tableCategories.replaceData(subcategories)
        ...
        function reloadTableFilter(id) {
            ...
            fetchFilterJSON().then(data => {
                ...
                // console.log(data)
                tableCategories.replaceData(data)
                // console.log(tableCategories)
            })
        }

        function addLinkBreadCumb(link, id, type) {
            ...
            console.log(allLinksBreadcumbCategories)
        ...
        // Seleccion a todos
        document.querySelector('.categoriesTable .items-actions .item.all').addEventListener("click", function () {
        tableCategories.deselectRow();
        tableCategories.selectRow();
        });
        // Selección página actual
        document.querySelector('.categoriesTable .items-actions .item.current').addEventListener("click", function () {
        tableCategories.deselectRow();
        tableCategories.selectRow("visible")
        });
        // Seleccion inactivos
        document.querySelector('.categoriesTable .items-actions .item.inactive').addEventListener("click", function () {
        tableCategories.deselectRow();
        let rows = tableCategories.getRows()
        rows.filter(function (row) {
            if (row._row.data.estado == 'inactive') {
                tableCategories.selectRow(row);
            }
        });
        });
        // Seleccion activos
        document.querySelector('.categoriesTable .items-actions .item.active').addEventListener("click", function () {
        tableCategories.deselectRow();
        let rows = tableCategories.getRows()
        console.log(rows)
        rows.filter(function (row) {
        ...
        // Action eliminar
        document.querySelector('.categoriesTable .items-actions .item.isDelete').addEventListener("click", function () {
        let rows = tableCategories.getRows()
        let rowsSelected = [];
        rows.filter(function (row) {
            if (row.isSelected()) {
        ...
        // Action desactivar
        document.querySelector('.categoriesTable .items-actions .item.isDesactive').addEventListener("click", function () {
        let rows = tableCategories.getRows()
        let rowsSelected = [];
        rows.filter(function (row) {
            if (row.isSelected()) {
        ...
        // Action activar
        document.querySelector('.categoriesTable .items-actions .item.isActive').addEventListener("click", function () {
        let rows = tableCategories.getRows()
        let rowsSelected = [];
        rows.filter(function (row) {
            if (row.isSelected()) {
        ...
        // Action duplicar
        document.querySelector('.categoriesTable .items-actions .item.isDuplicate').addEventListener("click", function () {
        let rows = tableCategories.getRows()
        let rowsSelected = [];
        rows.filter(function (row) {
            if (row.isSelected()) {
        ...
        if (confirm(`¿Está seguro de duplicar estas ${ rowsSelected.length } filas?`)) {
            rowsSelected.map(row => {
                tableCategories.addRow(Object.assign({}, row.getData()))
            })
        }
        });
        
        function deleteFilter(id, item = '') {
        let urlAllFilter = BASE_URL + 'Filter/delFilter/'+id
...
function loadTreeFilter() {
    let urlAllFilterParent = BASE_URL + 'Filter/getFilterParent'
    let arrX = [];
    let subcategoriesHTML = document.querySelector('.chevrondown-gj8 .chevrondown-content-gj8.not');
    subcategoriesHTML.innerHTML = '';
    subcategoriesHTML.parentElement.querySelector('.chevrondown-radio-button.not').classList.add('active')
    async function fetchFilterParentJSON() {
        const response = await fetch(urlAllFilterParent);
        const categories = await response.json();
        return categories;
    }
        ...
        tree.map(item => {
            ...
            <div class="chevrondown-item-gj8">
                <div class="chevrondown-head-gj8">
                    <i class="material-icons">chevron_right</i>
                    <span class="chevrondown-radio-button" item="${item.id}"></span>
                    ${ item.nombre } <span class="category-mark">(Categoría)</span>
                </div>
                <div class="chevrondown-content-gj8">
                    ${ viewSubCat(item.subcategory) }
                </div>
