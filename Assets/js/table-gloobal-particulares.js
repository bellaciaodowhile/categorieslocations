...
let tableParticulares = new Tabulator("#particularesTable", {
data: tabledataParticulares,
layout: "fitColumns",
...
rowFormatter: function (row) {
    ...
    let icon = document.createElement('i')
    if (row._row.table.getSelectedRows().length == tabledataParticulares.length) {
...
// Seleccion a todos
document.querySelector('.particularesTable .items-actions .item.all').addEventListener("click", function(){
tableParticulares.deselectRow();
tableParticulares.selectRow();
});
// Selección página actual
document.querySelector('.particularesTable .items-actions .item.current').addEventListener("click", function(){
tableParticulares.deselectRow();
tableParticulares.selectRow("visible")
});
// Seleccion inactivos
document.querySelector('.particularesTable .items-actions .item.inactive').addEventListener("click", function(){
tableParticulares.deselectRow();
let rows = tableParticulares.getRows()
rows.filter(function(row){
    if (row._row.data.col == 'inactive') {
        tableParticulares.selectRow(row);
    }
});
});
// Seleccion activos
document.querySelector('.particularesTable .items-actions .item.active').addEventListener("click", function(){
tableParticulares.deselectRow();
let rows = tableParticulares.getRows()
rows.filter(function(row){
...
// Action eliminar
document.querySelector('.particularesTable .items-actions .item.isDelete').addEventListener("click", function(){
let rows = tableParticulares.getRows()
let rowsSelected = [];
rows.filter(function(row){
    if (row.isSelected()) {
...
// Action desactivar
document.querySelector('.particularesTable .items-actions .item.isDesactive').addEventListener("click", function(){
let rows = tableParticulares.getRows()
let rowsSelected = [];
rows.filter(function(row){
    if (row.isSelected()) {
...
// Action activar
document.querySelector('.particularesTable .items-actions .item.isActive').addEventListener("click", function(){
let rows = tableParticulares.getRows()
let rowsSelected = [];
rows.filter(function(row){
    if (row.isSelected()) {
...
// Action duplicar
document.querySelector('.particularesTable .items-actions .item.isDuplicate').addEventListener("click", function(){
let rows = tableParticulares.getRows()
let rowsSelected = [];
rows.filter(function(row){
    if (row.isSelected()) {
...
if (confirm(`¿Está seguro de duplicar estas ${ rowsSelected.length } filas?`)) {
    rowsSelected.map(row => {
        tableParticulares.addRow(Object.assign({}, row.getData()))
    })
}
});
// console.clear();